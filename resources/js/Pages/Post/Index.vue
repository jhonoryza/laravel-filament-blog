            ...
            <div class="flex flex-col gap-4 mx-4">
                ...
                <form @submit.prevent="searching" class="flex items-center relative">
                    <input
                        ...
                        autocomplete="off"
                        class="form-input rounded px-1 py-1 text-sm text-slate-500 w-full
                             focus:ring-1 focus:ring-indigo-500 dark:bg-zinc-800 dark:text-white
                ...
                <div v-for="(post, index) in posts" :key="post.id" @click="goToDetail(post.slug)"
                      class="hover:opacity-60 flex flex-col border border-slate-100 shadow-xl rounded-xl hover:cursor-pointer
                            overflow-hidden pb-2 bg-white dark:bg-zinc-800 dark:text-white"
                >
                    <!-- image thumbnail -->
                    ...
                    <div class="flex flex-col justify-between px-4 gap-4">
                        ...
                        <!-- summary -->
                        <p v-show="post.summary !== ''" class="text-base text-slate-700"> {{ post.summary }} </p>
                        <!-- end summary -->

                        <!-- Tag -->
                        <div class="flex flex-wrap gap-4">
                            <span v-for="catName in post.categories" class=" border border-indigo-200 rounded-lg shadow-xl
                                text-sm text-slate-700 dark:text-white font-medium py-1 px-2">
                                {{ catName }}
                            </span>
                        </div>
                        <!-- end Tag -->
                        ...
                        <div class="flex justify-between">
                            <p class="text-sm  text-slate-900 dark:text-white">[{{ index + 1 }}]</p>
                ...
                <button
                    ...
                    @click="scrollTop"
                    class="fixed bottom-5 right-1/4 bg-indigo-100 rounded-full p-3 shadow-lg
                            hover:opacity-60 focus:outline-none z-50"
                    style="display: none;"
                >
                    ⬆️
                </button>
